...
def fixed_expenses_status():
    ...
    rows=[dict(x) for x in c.execute(f"SELECT * FROM fixed_expenses WHERE active={active_value} ORDER BY due_day ASC, name ASC").fetchall()]
    for r in rows:
        r['paid']=False; r['paid_value']=0
...
def fixed_expense_pay(item_id):
    d=request.json or {}; paydate=str(d.get('date') or datetime.date.today().isoformat())[:10]
    payment=str(d.get('payment') or '').strip(); c=db()
    active_value = 'TRUE' if USE_POSTGRES else '1'
    r=c.execute(f'SELECT * FROM fixed_expenses WHERE id=? AND active={active_value}',(item_id,)).fetchone()
    if not r:
        c.close(); return jsonify(error='Conta fixa não encontrada.'),404
    desc=f"Conta fixa: {r['name']}"; month=paydate[:7]
    ...
    if not r['recurring']:
        c.execute('UPDATE fixed_expenses SET active=? WHERE id=?',(False if USE_POSTGRES else 0,item_id))
