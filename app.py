...
TABLES={'users','vehicles','services','appointments','orders','stock','finance','followups','budgets','order_items','stock_moves','order_photos','order_payments','audit_log','company','fixed_expenses'}
...
def init():
    if USE_POSTGRES:
        ...
        try:
            ...
            c.execute("CREATE TABLE IF NOT EXISTS fixed_expenses(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name TEXT NOT NULL, category TEXT DEFAULT 'Outros', amount DOUBLE PRECISION DEFAULT 0, due_day INTEGER DEFAULT 10, payment TEXT DEFAULT 'Pix', recurring BOOLEAN DEFAULT TRUE, active BOOLEAN DEFAULT TRUE, created_at TEXT DEFAULT '')")
    ...
    CREATE TABLE IF NOT EXISTS fixed_expenses(id INTEGER PRIMARY KEY, name TEXT NOT NULL, category TEXT DEFAULT 'Outros', amount REAL DEFAULT 0, due_day INTEGER DEFAULT 10, payment TEXT DEFAULT 'Pix', recurring INTEGER DEFAULT 1, active INTEGER DEFAULT 1, created_at TEXT DEFAULT '');
    ''')
    # migrations from V12
...
def generic(table):
    ...
    if USE_POSTGRES and table == 'fixed_expenses' and 'recurring' in data:
        v=data.get('recurring')
        if isinstance(v, (int,float)): data['recurring']=bool(v)
        elif isinstance(v,str) and v.strip().lower() in ('0','1','true','false'):
            data['recurring']=v.strip().lower() in ('1','true')
    try:
...
@app.get('/api/fixed-expenses/status')
def fixed_expenses_status():
    month=(request.args.get('month') or datetime.date.today().strftime('%Y-%m'))[:7]
    c=db()
    rows=[dict(x) for x in c.execute("SELECT * FROM fixed_expenses WHERE active=TRUE ORDER BY due_day ASC, name ASC").fetchall()]
    for r in rows:
        r['paid']=False
        r['paid_value']=0
        found=c.execute("SELECT COALESCE(SUM(value),0) total FROM finance WHERE kind='Saída' AND category='Conta fixa' AND description=? AND date LIKE ?",(f"Conta fixa: {r['name']}",month+'%')).fetchone()
        if found and float(found['total'] or 0)>0:
            r['paid']=True; r['paid_value']=float(found['total'] or 0)
    c.close(); return jsonify(rows)

@app.post('/api/fixed-expenses/<int:item_id>/pay')
def fixed_expense_pay(item_id):
    d=request.json or {}
    paydate=str(d.get('date') or datetime.date.today().isoformat())[:10]
    payment=str(d.get('payment') or '').strip()
    c=db()
    r=c.execute('SELECT * FROM fixed_expenses WHERE id=? AND active=TRUE',(item_id,)).fetchone()
    if not r:
        c.close(); return jsonify(error='Conta fixa não encontrada.'),404
    desc=f"Conta fixa: {r['name']}"
    month=paydate[:7]
    exists=c.execute("SELECT id FROM finance WHERE kind='Saída' AND category='Conta fixa' AND description=? AND date LIKE ? LIMIT 1",(desc,month+'%')).fetchone()
    if exists:
        c.close(); return jsonify(error='Esta conta já foi lançada como paga neste mês.'),400
    value=float(d.get('value') or r['amount'] or 0)
    if value<=0:
        c.close(); return jsonify(error='Informe um valor maior que zero.'),400
    method=payment or r['payment'] or 'Outro'
    c.execute("INSERT INTO finance(date,kind,description,value,payment,category,order_id) VALUES(?,?,?,?,?,?,?)",(paydate,'Saída',desc,value,method,'Conta fixa',0))
    if not r['recurring']:
        c.execute('UPDATE fixed_expenses SET active=FALSE WHERE id=?',(item_id,))
